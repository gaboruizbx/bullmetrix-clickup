...
async function doLogin() {
  ...
  try {
    ...
    apiKey = k;
    sessionStorage.setItem('bm_key', k);
    document.getElementById('user-name').textContent = data.user.username || data.user.email;
    var initials = (data.user.username || 'U').substring(0, 2).toUpperCase();
    document.getElementById('user-initials').textContent = initials;
    document.getElementById('login-screen').classList.add('hidden');
    document.getElementById('main-app').classList.remove('hidden');
  } catch(e) {
    err.textContent = 'Error conectando: ' + e.message;
  }
}

function doLogout() {
  sessionStorage.removeItem('bm_key');
  apiKey = '';
...
window.addEventListener('load', function() {
  var saved = sessionStorage.getItem('bm_key');
  if (saved) {
...
async function crearTarea() {
  ...
  var toast  = document.getElementById('toast-crear');
  toast.className = 'toast'; toast.innerHTML = '';
  ...
  try {
    ...
    if (minutos > 0) {
      var dur = minutos * 60000;
      await ck('/task/' + data.id + '/time', { method: 'POST', body: JSON.stringify({ start: Date.now() - dur, duration: dur }) });
    }
...
function limpiarFormCrear() {
  ...
  ['c-lista','c-tipo','c-sub','c-pod','c-prioridad'].forEach(function(id) {
...
async function cargarTarea() {
  ...
  try {
    ...
    showToast('toast-editar', 'err', 'Error cargando: ' + e.message);
  }
}

async function guardarEdicion() {
  if (!currentTaskId) return;
  var toast = document.getElementById('toast-editar');
  toast.className = 'toast'; toast.innerHTML = '';
  ...
  try {
    var data = await ck('/task/' + currentTaskId, { method: 'PUT', body: JSON.stringify(body) });
    var minutos = parseInt(document.getElementById('e-tiempo').value) || 0;
    if (minutos > 0) {
      var dur = minutos * 60000;
      await ck('/task/' + currentTaskId + '/time', { method: 'POST', body: JSON.stringify({ start: Date.now() - dur, duration: dur }) });
    }
...
async function buscar() {
  ...
  try {
    // Buscar en la API con el query completo
    var data = await ck('/team/' + WORKSPACE + '/task?query=' + encodeURIComponent(q) + '&assignees[]=88327341&page=0&subtasks=true&include_closed=true');
    var tasks = data.tasks || [];

    // Filtrar client-side por términos parciales
    ...
    // Si no matchea exacto, mostrar todos los resultados de la API igual
    var toShow = filtered.length > 0 ? filtered : tasks;

    if (!toShow.length) {
      cont.innerHTML = '<div class="empty-state"><p>Sin resultados para "' + esc(q) + '"</p></div>';
      return;
    }
    ...
    toShow.slice(0, 15).forEach(function(t) {
      ...
      card.innerHTML =
        '<div class="tname">' + highlightMatch(esc(t.name), words) + '</div>' +
        '<div class="tmeta">' +
          '<span class="status-pill ' + stClass + '">' + esc(st) + '</span>' +
          (t.list ? '<span>' + esc(t.list.name) + '</span>' : '') +
          '<a href="' + t.url + '" target="_blank">ver</a>' +
          '<button class="edit-link" onclick="cargarParaEditar(\'' + t.id + '\')">editar</button>' +
        '</div>';
    ...
    if (toShow.length > 15) {
      ...
      more.textContent = 'Mostrando 15 de ' + toShow.length + ' resultados. Refiná la búsqueda para ver más.';
...
function addHist(name, url, action) {
  ...
  item.innerHTML =
    '<span class="hname">' + esc(name) + '<span class="haction">' + action + '</span></span>' +
